 ...
 * e.g. managed by {@link DataSourceTransactionManager} or {@link org.springframework.transaction.jta.JtaTransactionManager}.
 *
...
public abstract class DataSourceUtils {
   ...
   * when using {@link DataSourceTransactionManager}. Will bind a Connection to the thread if
   * transaction synchronization is active, e.g. when running within a {@link
   ...
   * <p>Is aware of a corresponding Connection bound to the current thread, for example
   * when using {@link DataSourceTransactionManager}. Will bind a Connection to the thread if
   * transaction synchronization is active (e.g. if in a JTA transaction).
  ...
  public static void doCloseConnection(Connection con, DataSource dataSource) throws SQLException {
    if (!(dataSource instanceof SmartDataSource) || ((SmartDataSource) dataSource)
        .shouldClose(con)) {
      con.close();
    }
  }
