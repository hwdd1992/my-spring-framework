 ...
 * or context; that is to say it is not coupled to validating only objects in the web tier, the
 * data-access tier, or the whatever-tier. As such it is amenable to being used in any layer of an
 * application, and supports the encapsulation of validation logic as a first-class citizen in its
 * own right.
 *
 * <p>Find below a simple but complete {@code Validator}
 * implementation, which validates that the various {@link String} properties of a {@code UserLogin}
 * instance are not empty (that is they are not {@code null} and do not consist wholly of
 * whitespace), and that any password that is present is at least {@code 'MINIMUM_PASSWORD_LENGTH'}
 ...
 * the {@code Validator} interface and its role in an enterprise application.
 *
 * @author Rod Johnson
 * @see Errors
 * @see ValidationUtils
