 ...
 * to parse some arbitrarily complex XML into one or more {@link BeanDefinition BeanDefinitions}. If
 * you just want to parse some XML into a single {@code BeanDefinition}, you may wish to consider
 * the simpler convenience extensions of this class, namely {@link AbstractSingleBeanDefinitionParser}
 * and {@link AbstractSimpleBeanDefinitionParser}.
 *
 * @author Rob Harrop
...
public abstract class AbstractBeanDefinitionParser implements BeanDefinitionParser {
   ...
   * parameter is {@code false}, because one typically does not want inner beans to be registered as
   * top level beans.
   *
   * @param definition the bean definition to be registered
   ...
   * Note that this flag is about <i>always</i> generating an ID; the parser won't even check for an
   ...
   * <p>Disabled by default; subclasses can override this to enable ID generation
   * as fallback: The parser will first check for an "id" attribute in this case, only falling back
   * to a generated ID if no value was specified.
   *
   ...
   * an event will be fired when a bean definition has been completely parsed. Override this to
   * return {@code false} in order to suppress the event.
   *
