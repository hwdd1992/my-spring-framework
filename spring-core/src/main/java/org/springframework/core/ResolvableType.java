...
public class ResolvableType implements Serializable {
  ...
  /**
   * Return the underling Java {@link Type} being managed. With the exception of the {@link #NONE}
   * constant, this method will never return {@code null}.
   */
  public Type getType() {
   ...
   * whether both the {@link #resolve() resolved} {@code Class} is {@link
   * Class#isAssignableFrom(Class) assignable from} the given type as well as whether all {@link
   * #getGenerics() generics} are assignable.
   *
   ...
   * type; 2 indicates the first nested generic; 3 the second; and so on. For example, given {@code
   * List<Set<Integer>>} level 1 refers to the {@code List}, level 2 the {@code Set}, and level 3
   * the {@code Integer}.
   ...
   *
   * @return an array of {@link ResolvableType}s representing the generic parameters (never {@code
   * null})
   * @see #hasGenerics()
   * @see #getGeneric(int...)
   * @see #resolveGeneric(int...)
  ...
  public ResolvableType[] getGenerics() {
    ...
    if (this.generics == null) {
      if (this.type instanceof Class) {
        ...
        this.generics = forTypes(SerializableTypeWrapper.forTypeParameters(typeClass),
            this.variableResolver);
   ...
   * type cannot be resolved. This method will consider bounds of {@link TypeVariable}s and {@link
   * WildcardType}s if direct resolution fails; however, bounds of {@code Object.class} will be
   * ignored.
   *
   * @param fallback the fallback class to use if resolution fails (may be {@code null})
  ...
  private ResolvableType resolveVariable(TypeVariable<?> variable) {
    ...
    if (this.type instanceof ParameterizedType) {
      ...
      if (parameterizedType.getOwnerType() != null) {
        return forType(parameterizedType.getOwnerType(), this.variableResolver)
  ...
  public boolean equals(Object other) {
    ...
    if (this.typeProvider != otherType.typeProvider &&
        (this.typeProvider == null || otherType.typeProvider == null ||
            !ObjectUtils
                .nullSafeEquals(this.typeProvider.getType(), otherType.typeProvider.getType()))) {
      return false;
    }
   ...
   * information for assignability checks. For example: {@code ResolvableType.forClass(MyArrayList.class)}.
   *
  ...
  static void resolveMethodParameter(MethodParameter methodParameter) {
    ...
    methodParameter.setParameterType(
        forType(null, new MethodParameterTypeProvider(methodParameter), owner.asVariableResolver())
            .resolve());
  }
  ...
  /**
   * Return a {@link ResolvableType} for the specified {@link Type}. Note: The resulting {@link
   * ResolvableType} may not be {@link Serializable}.
   *
  ...
  /**
   * Return a {@link ResolvableType} for the specified {@link ParameterizedTypeReference}. Note: The
   ...
   * @return a {@link ResolvableType} for the specified {@link Type} and {@link VariableResolver}
   */
  static ResolvableType forType(Type type, TypeProvider typeProvider,
